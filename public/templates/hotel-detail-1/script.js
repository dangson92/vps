(function(){
  ...
  var bc=document.getElementById('breadcrumb');if(bc){var items=(data.breadcrumb_items||[]);var paths=(data.breadcrumb_paths||[]);var h='';var mainUrl=(data.main_domain_url||'');for(var i=0;i<items.length;i++){var it=items[i];var last=i===items.length-1;var sep=i>0?'<span class="text-neutral-700 text-sm font-medium">/</span>':'';var url='';if(mainUrl&&paths[i]){url=mainUrl+paths[i];}var node=last?'<span class="text-neutral-900 text-sm font-medium">'+it+'</span>':(url?'<a class="text-neutral-700 text-sm font-medium hover:text-primary transition-colors" href="'+url+'">'+it+'</a>':'<span class="text-neutral-700 text-sm font-medium">'+it+'</span>');h+=sep+node;}bc.innerHTML=h;}
