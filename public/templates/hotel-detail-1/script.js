(function(){
  ...
  var am=document.getElementById('amenities-grid');if(am){var arr=(data.amenities||[]);var iconData=function(s){var v=(s||'').toLowerCase();if(/wifi|internet/.test(v)) return {icon:'wifi',cls:'amenity-wifi'};if(/air|conditioning|ac/.test(v)) return {icon:'ac_unit',cls:'amenity-air'};if(/tv|television|cable|satellite/.test(v)) return {icon:'tv',cls:'amenity-tv'};if(/spa|wellness/.test(v)) return {icon:'spa',cls:'amenity-spa'};if(/restaurant|snack|breakfast|minibar|bar/.test(v)) return {icon:'restaurant',cls:'amenity-restaurant'};if(/security|alarm|extinguisher|smoke/.test(v)) return {icon:'security',cls:'amenity-security'};if(/shuttle|airport/.test(v)) return {icon:'airport_shuttle',cls:'amenity-shuttle'};if(/elevator|lift/.test(v)) return {icon:'elevator',cls:'amenity-elevator'};if(/disabled|accessible|wheelchair/.test(v)) return {icon:'accessible',cls:'amenity-accessible'};if(/heating|thermo|fireplace/.test(v)) return {icon:'local_fire_department',cls:'amenity-heating'};if(/laundry|ironing|dry cleaning/.test(v)) return {icon:'local_laundry_service',cls:'amenity-laundry'};if(/meeting|business|fax|photocopying/.test(v)) return {icon:'business_center',cls:'amenity-business'};if(/lockers|safe|deposit/.test(v)) return {icon:'lock',cls:'amenity-lock'};if(/view|scenic|terrace|patio|balcony/.test(v)) return {icon:'landscape',cls:'amenity-view'};if(/room service/.test(v)) return {icon:'room_service',cls:'amenity-room-service'};if(/refrigerator|kettle|electric|kitchen/.test(v)) return {icon:'kitchen',cls:'amenity-kitchen'};if(/bathroom|toilet|shower|bathtub/.test(v)) return {icon:'bathroom',cls:'amenity-default'};if(/towel|linen/.test(v)) return {icon:'dry_cleaning',cls:'amenity-laundry'};if(/wardrobe|closet/.test(v)) return {icon:'checkroom',cls:'amenity-default'};if(/desk/.test(v)) return {icon:'desk',cls:'amenity-business'};if(/telephone|phone/.test(v)) return {icon:'call',cls:'amenity-default'};if(/fruit/.test(v)) return {icon:'nutrition',cls:'amenity-restaurant'};if(/tour|cycling|walking|bicycle/.test(v)) return {icon:'directions_bike',cls:'amenity-shuttle'};if(/karaoke|entertainment|music/.test(v)) return {icon:'music_note',cls:'amenity-spa'};if(/parking|car/.test(v)) return {icon:'local_parking',cls:'amenity-shuttle'};if(/pool|swimming/.test(v)) return {icon:'pool',cls:'amenity-spa'};if(/gym|fitness/.test(v)) return {icon:'fitness_center',cls:'amenity-spa'};if(/pet/.test(v)) return {icon:'pets',cls:'amenity-default'};if(/smoking/.test(v)) return {icon:'smoking_rooms',cls:'amenity-default'};if(/non-smoking/.test(v)) return {icon:'smoke_free',cls:'amenity-security'};if(/family/.test(v)) return {icon:'family_restroom',cls:'amenity-default'};if(/24-hour|front desk/.test(v)) return {icon:'support_agent',cls:'amenity-business'};if(/housekeeping/.test(v)) return {icon:'cleaning_services',cls:'amenity-laundry'};if(/currency|atm/.test(v)) return {icon:'currency_exchange',cls:'amenity-business'};if(/luggage/.test(v)) return {icon:'luggage',cls:'amenity-shuttle'};return {icon:'check_circle',cls:'amenity-default'};};var ah='';for(var a=0;a<arr.length;a++){var it=arr[a];var d=iconData(it);ah+='<div class="flex items-center gap-3"><span class="material-symbols-outlined '+d.cls+'">'+d.icon+'</span><span class="text-neutral-900">'+it+'</span></div>';}
    am.innerHTML=ah;}
