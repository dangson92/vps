(function(){
  ...
  var data=window.HOTEL_DATA||{};if(!data||!data.title){try{var pd=document.getElementById('page-data');if(pd){data=JSON.parse(pd.textContent||'{}')||{};}}catch(e){data={}}}
  ...
  var rs=document.getElementById('rating-stars');if(rs){var rating=parseFloat(data.rating||0);if(rating>0){var full=Math.floor(rating);var hasHalf=rating%1>=0.5;var empty=5-full-(hasHalf?1:0);var stars='';for(var s=0;s<full;s++){stars+='<span class="material-symbols-outlined text-accent-gold" style="font-variation-settings:\'FILL\' 1">star</span>';}if(hasHalf){stars+='<span class="material-symbols-outlined text-accent-gold">star_half</span>';}for(var e=0;e<empty;e++){stars+='<span class="material-symbols-outlined text-gray-300">star</span>';}rs.innerHTML=stars;}}
